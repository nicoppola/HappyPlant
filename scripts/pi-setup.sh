...
if command -v influxd &>/dev/null; then
    ...
    curl -sfLo "/tmp/${INFLUX_DEB}" "https://dl.influxdata.com/influxdb/releases/${INFLUX_DEB}"
    [[ -s "/tmp/${INFLUX_DEB}" ]] || fail "Failed to download InfluxDB server .deb"
    dpkg -i "/tmp/${INFLUX_DEB}" || apt install -f -y
    rm -f "/tmp/${INFLUX_DEB}"
...
if command -v influx &>/dev/null; then
    ...
    TMP_DIR=$(mktemp -d)
    curl -sfLo "$TMP_DIR/$INFLUX_CLI_TAR" "https://dl.influxdata.com/influxdb/releases/${INFLUX_CLI_TAR}"
    [[ -s "$TMP_DIR/$INFLUX_CLI_TAR" ]] || fail "Failed to download InfluxDB CLI"
    tar -xzf "$TMP_DIR/$INFLUX_CLI_TAR" -C "$TMP_DIR"
    mv "$TMP_DIR/influx" /usr/local/bin/
    rm -rf "$TMP_DIR"
...
if [[ -n "$ALREADY_SETUP" ]]; then
    ...
    echo "  Need your InfluxDB admin password to retrieve the API token."
    read -rsp "  Password for user 'happyplant': " EXISTING_PASSWORD
    echo

    # Sign in to get a session cookie, then list authorizations
    SESSION=$(curl -sf -X POST http://localhost:8086/api/v2/signin \
        -u "happyplant:$EXISTING_PASSWORD" \
        -c - 2>/dev/null) || fail "Failed to sign in to InfluxDB. Wrong password?"

    INFLUX_TOKEN=$(curl -sf http://localhost:8086/api/v2/authorizations \
        -b <(echo "$SESSION") 2>/dev/null \
        | grep -o '"token":"[^"]*"' | head -1 | cut -d'"' -f4 || echo "")

    if [[ -z "$INFLUX_TOKEN" ]]; then
        echo "  Could not auto-detect token."
        read -rp "  Enter your InfluxDB API token manually: " INFLUX_TOKEN
    fi
else
    ...
    # The setup response returns the token in auth.token
    INFLUX_TOKEN=$(echo "$SETUP_RESPONSE" | grep -o '"token":"[^"]*"' | tail -1 | cut -d'"' -f4 || echo "")
    ...
    # Configure the CLI so 'influx' commands work going forward
    influx config create --config-name default \
        --host-url http://localhost:8086 \
        --org Home \
        --token "$INFLUX_TOKEN" --active >/dev/null 2>&1 || true
fi

[[ -n "$INFLUX_TOKEN" && "$INFLUX_TOKEN" != "REPLACE_ME" ]] || fail "No valid InfluxDB token. Cannot continue."
